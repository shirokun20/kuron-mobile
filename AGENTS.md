...
### Claude Code
- **Instruction file: this `AGENTS.md`** — Claude Code membacanya sebagai project memory. Repo ini sengaja **tidak** punya `CLAUDE.md` lagi (dihapus 2026-09-26) supaya tidak ada dua sumber aturan. Kalau versi Claude Code bos tak membaca `AGENTS.md`, buat symlink, bukan file terpisah: `ln -s AGENTS.md CLAUDE.md`.
...
- **Single source of truth**: this `AGENTS.md` only. Tidak ada `CLAUDE.md` maupun `.github/copilot-instructions.md` di repo — jangan buat lagi, dan jangan pakai `.cursorrules`/`.github/instructions` sebagai aturan terpisah. Butuh tool tertentu? Tambah di `.github/agents/` (agent/chatmode) atau `.github/prompts/`, bukan file instruction root.
...
- **Release**: `./build_release.sh` (Custom). Platform proyek **Android-only** (tak ada `ios/`), jadi `flutter build ipa` tidak relevan.
...
- **DI**: `get_it` via `core/di/service_locator.dart` — **manual registration** (`registerLazySingleton`/`registerFactory`). Tidak ada `injectable`/generated DI: jangan menambahkannya, edit `service_locator.dart` langsung.
...
└── core/
    └── di/          # get_it registration (service_locator.dart)
```

`lib/domain/` harus tetap pure Dart — `rg -l "package:flutter" lib/domain` harus kosong. Kalau butuh Flutter di domain, artinya logic-nya salah tempat.

### Naming & Style
- **Files**: `snake_case`
- **Classes**: `PascalCase`
- **Variables**: `camelCase`
- **Imports**: Dart SDK → Flutter → external packages → project imports (konvensi, belum di-enforce lint)

### State Management
- **State**: `flutter_bloc` (complex) or `Cubit` (simple). **All Cubits MUST extend `BaseCubit`.**
...
If you need to modify generated code, edit the source file and run `fvm flutter pub run build_runner build --delete-conflicting-outputs`.
...
## 🚀 Quality & Performance
- **Git**: Conventional Commits (`feat(auth): msg`). Branches: `master` (prod), `develop`, `feature/*`, `fix/*`, `hotfix/*`.
- **Perf**: `const` widgets, `ListView.builder` (never list children), lazy load >50 items.
- **Benchmark**: selalu **warmup dulu** sebelum mengukur. Angka run pertama = biaya JIT/AOT compile, bukan performa runtime (terbukti: parser 78ms → 12ms setelah warmup). Ukur median dari ≥5 run.
...
### Dormant Code (jangan di-revive tanpa alasan eksplisit)
- **Rust `image_process_single` / `image_split`** (`packages/kuron_native/rust/src/lib.rs` + `rust_bridge.dart`): 0 caller dari Dart sejak rantai PDF dihapus. Re-enable hanya kalau pipeline mosaic `reader-ai-learning-mode` butuh, dan `ImageSplitter` jadi bottleneck. Prasyarat: uji FFI di isolate (`DynamicLibrary.open` per isolate) di device — lookup di-comment-out (575ebca8) karena `RustBridge` null di compute isolate.
- **HTML parser → Rust**: sudah Putus (YAGNI), benchmark warmup+median 7×: detail nicomanga 250KB = 12ms, reader 1.1MB = 17ms, home = 1-3ms. Tidak perlu dibuka lagi tanpa data baru.

## 📦 Release Policy
- **Ver**: `MAJOR.MINOR.PATCH+BUILD`. Update `pubspec`, `CHANGELOG.md`, `README.md`.
- **Check**: Pass `fvm flutter test` & `fvm flutter analyze`. No sensitive data logs. Staged rollout.
